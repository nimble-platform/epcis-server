...
public class ResourceResolver implements LSResourceResolver {
	...
	public LSInput resolveResource(String type, String namespaceURI, String publicId, String systemId, String baseURI) {
		...
		if (!pathMap.containsKey(currentResourceName)) {
			...
			String s1 = s.next().replaceAll("\\n", " ") // the parser cannot understand elements broken down multiple
														// lines e.g. (<xs:element \n name="buxing">)
					.replace("\\t", " ") // these two about whitespaces is only for decoration
					.replaceAll("\\s+", " ").replaceAll("[^\\x20-\\x7e]", ""); // some files has a special character as
																				// a first character indicating utf-8
																				// file
		...
		// If this resource has already been added, do not add the same resource again.
		// It throws
		// "org.xml.sax.SAXParseException: sch-props-correct.2: A schema cannot contain
